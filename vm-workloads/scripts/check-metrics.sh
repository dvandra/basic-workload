...
# Check Thanos metrics for VM workloads
# Queries acm_rs_vm:namespace:* metrics from the Hub cluster
#
...
echo "=========================================="
echo "VM Workload Metrics Check"
echo "=========================================="
echo "Namespace: $NAMESPACE"
echo "Metrics: acm_rs_vm:namespace:*"
echo ""
...
query_metric() {
    local metric=$1
    local query="last_over_time(acm_rs_vm:namespace:${metric}{namespace=\"${NAMESPACE}\"}[30m])"
...
echo "Step 1: Checking if acm_rs_vm:namespace:* metrics are registered in Thanos..."
global_check=$(curl -s -k --connect-timeout 10 --max-time 30 -G \
    --data-urlencode "query=count(last_over_time(acm_rs_vm:namespace:cpu_request[30m]))" \
    -H "Authorization: Bearer $(oc whoami -t)" \
    "${THANOS_ENDPOINT}/api/v1/query" 2>&1)

if echo "$global_check" | grep -q '"result":\[\]'; then
    echo -e "${RED}✗ acm_rs_vm:namespace:* metrics NOT FOUND in Thanos${NC}"
    echo ""
    echo "Please verify:"
    echo "  1. ACM Operator is installed on Hub (version 2.14+)"
    echo "  2. Multicluster Observability (MCO) is deployed on Hub"
    echo "  3. OpenShift Virtualization is installed on spoke cluster"
    echo "  4. Spoke cluster is registered and connected"
    exit 1
else
    echo -e "${GREEN}✓ VM metrics are registered in Thanos${NC}"
fi
echo ""
...
ns_check=$(curl -s -k --connect-timeout 10 --max-time 30 -G \
    --data-urlencode "query=last_over_time(acm_rs_vm:namespace:cpu_usage{namespace=\"${NAMESPACE}\"}[30m])" \
    -H "Authorization: Bearer $(oc whoami -t)" \
    "${THANOS_ENDPOINT}/api/v1/query" 2>&1)

if echo "$ns_check" | grep -q '"result":\[\]'; then
    echo -e "${YELLOW}⚠ No data found for namespace '${NAMESPACE}'${NC}"
    echo "VMs may need more time to generate metrics (wait 15-20 minutes)"
else
...
for metric in "${METRICS[@]}"; do
    ...
    if [ "$value" != "N/A" ] && [ -n "$value" ]; then
        printf "%-45s ${GREEN}%-20s${NC}\n" "acm_rs_vm:namespace:${metric}" "$value"
    else
        printf "%-45s ${RED}%-20s${NC}\n" "acm_rs_vm:namespace:${metric}" "N/A"
    fi
done
